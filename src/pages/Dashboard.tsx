    ...
    <div className="min-h-screen gradient-desert px-4 sm:px-6 py-8 pb-28" dir="rtl">
      <div className="max-w-2xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-between mb-6">
          ...
          <Link to="/settings" className="p-2.5 text-muted-foreground hover:text-foreground transition-colors rounded-lg hover:bg-card">
        ...
        {isLowPerformance && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}
            className="bg-primary/5 border border-primary/20 rounded-xl p-4 mb-5">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className="bg-card border border-border rounded-xl p-8 mb-5 text-center shadow-sand">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            ...
            <p className="text-muted-foreground text-xs mt-1 font-sans-ui">التقييم</p>
          </motion.div>
        </div>

        {/* Heatmap */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
          className="bg-card border border-border rounded-xl p-5 mb-5 shadow-sand">
          ...
          <div className="grid grid-cols-7 gap-1.5">
            {heatmap.map((d) => {
              const intensity = d.score === 0 ? 'bg-border' :
                ...
                d.score <= 25 ? 'bg-accent/40' :
                d.score <= 35 ? 'bg-primary/40' : 'bg-primary/80';
              return (
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-card/95 backdrop-blur-sm border-t border-border px-4 py-3 z-50">
        <div className="max-w-2xl mx-auto flex justify-around">
          <Link to="/dashboard" className="flex flex-col items-center gap-1 text-primary">
            ...
            <span className="text-xs font-sans-ui">الرحلة</span>
          </Link>
          <Link to="/progress" className="flex flex-col items-center gap-1 text-muted-foreground hover:text-foreground transition-colors">
            ...
            <span className="text-xs font-sans-ui">التقييم</span>
          </Link>
          <Link to="/statistics" className="flex flex-col items-center gap-1 text-muted-foreground hover:text-foreground transition-colors">
            ...
            <span className="text-xs font-sans-ui">الإحصائيات</span>
          </Link>
          <Link to="/weekly" className="flex flex-col items-center gap-1 text-muted-foreground hover:text-foreground transition-colors">
            ...
            <span className="text-xs font-sans-ui">التقرير</span>
          </Link>
        </div>
      </nav>
    </div>
